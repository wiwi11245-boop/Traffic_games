// 🛠️ 完整動態版 end.js：防手機搶跑、畫面完全渲染就緒後才啟動 12s 車輛與音效時序
...
function renderEndScene(container, onReturnToTitle) {
    ...
    if (!document.getElementById('css-scene-end-gpu-optimized')) {
        ...
        style.textContent = `
            ...
            .scene-end-viewport.active-play .setting-sun {
            ...
            .car-frame1 {
                ...
                transform: translate3d(-250px, 0, 0); /* 初始停在畫面外 */
                will-change: transform, opacity;
            }
            .scene-end-viewport.active-play .car-frame1 {
            ...
            .car-frame2 {
                ...
                transform: translate3d(900px, -70px, 0);
                will-change: transform, opacity;
            }
            .scene-end-viewport.active-play .car-frame2 {
            ...
            .car-frame3 {
                ...
                transform: translate3d(770px, -160px, 0);
                will-change: transform, opacity;
            }
            .scene-end-viewport.active-play .car-frame3 {
            ...
            .car-frame4 {
                ...
                transform: translate3d(843px, -160px, 0);
                will-change: transform, opacity;
            }
            .scene-end-viewport.active-play .car-frame4 {
            ...
            .scene-end-viewport.active-play .dark-overlay {
            ...
            .scene-end-viewport.active-play .thanks-banner {
            ...
            .scene-end-viewport.active-play .return-title-banner {
    ...
    // 渲染結局 DOM 結構
    container.innerHTML = `
        <div id="endViewport" class="scene-end-viewport">
    ...
    // 🛠️ 雙重 rAF ＋ 轉場緩衝：確保手機端修車廠背景與 DOM 完全繪製到螢幕後，才正式啟動動畫與音樂
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            setTimeout(() => {
                ...
                if (endViewport) {
                    // ⚡ 正式開跑 12 秒動畫
                    endViewport.classList.add('active-play');
                }

                // 🎵 1. 同步從 0 秒開始播放 end.mp3
                audioEnd.currentTime = 0;
                audioEnd.play().catch(err => console.warn("end 音樂播放受阻:", err));

                // 🎵 2. 精確在動畫 70%（8400ms）時切換播放 結局音樂.mp3
                setTimeout(() => {
                    audioEnd.pause();
            ...
            }, 500); // 留給前一個轉場黑幕完全拉開與圖片準備的 500ms 緩衝
        });
    });
